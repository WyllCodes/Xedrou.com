...
// ---- token storage (localStorage-backed, in-memory cache on top) ----
let session = null; // { access_token, refresh_token, expires_at }
let refreshing = null; // in-flight refresh promise, so concurrent 401s share one refresh
const listeners = new Set(); // auth-state-changed subscribers (used by AuthContext)
...
// ---- realtime (Socket.IO, proxied through Express — see backend/src/realtime.ts) ----
let socket = null;
function getSocket() {
...
// Reconnect the socket with a fresh token whenever the session changes.
...
function makeEntityClient(entityName) {
  ...
  return {
    ...
    // Real-time updates, shaped like { type: 'create'|'update'|'delete', data } to match
    // what Distribution.jsx / Notifications.jsx / ArtistCatalog.jsx already expect.
    // Routed entirely through our own Socket.IO server (backend/src/realtime.ts) —
    // the browser never opens a connection to Supabase.
    subscribe: (callback) => {
      const s = getSocket();
      const tableEvent = `change:${entityName}`;
      const handler = (payload) => callback(payload);

      const join = () => s.emit("subscribe", entityTable(entityName));
      s.on("connect", join);
      if (s.connected) join();
      s.on(tableEvent, handler);

      return () => {
        s.off(tableEvent, handler);
        s.off("connect", join);
        s.emit("unsubscribe", entityTable(entityName));
      };
    },
  };
}

// Mirrors backend/scripts/gen-schema.js table-naming convention, only needed here to
// tell the realtime server which room to join.
function toSnakeCase(str) {
  return str.replace(/([a-z0-9])([A-Z])/g, "$1_$2").replace(/([A-Z])([A-Z][a-z])/g, "$1_$2").toLowerCase();
}
function entityTable(entityName) {
  if (entityName === "User") return "user_profiles";
  return toSnakeCase(entityName) + "s";
}

// base44.entities.AnyEntityName -> generated on first access, cached after that.
const entityCache = {};
const entities = new Proxy(
  {},
  {
    get(_target, entityName) {
      ...
      if (!entityCache[entityName]) entityCache[entityName] = makeEntityClient(entityName);
...
const auth = {
  ...
  loginWithProvider: async (provider, redirectPath = "/dashboard") => {
    const { url } = await apiFetch(
      `/api/auth/oauth-url?provider=${encodeURIComponent(provider)}&redirect_path=${encodeURIComponent(redirectPath)}`
    );
  ...
  verifyOtp: async ({ email, otpCode }) => {
    const data = await apiFetch("/api/auth/verify-otp", { method: "POST", body: { email, otpCode } });
    saveSession(data.session);
  ...
  resetPassword: async ({ newPassword }) => {
    // Supabase's recovery email links to /reset-password#access_token=...&type=recovery.
    // ResetPassword.jsx reads that token out of the URL hash and passes it through here.
    const hashParams = new URLSearchParams(window.location.hash.replace(/^#/, ""));
    const recoveryAccessToken = hashParams.get("access_token");
    return apiFetch("/api/auth/reset-password", { method: "POST", body: { recoveryAccessToken, newPassword } });
  },

  setToken: async (accessToken) => {
    // Kept for interface compatibility with any old call sites; session storage is
    // otherwise managed internally via saveSession().
    const current = loadSession() || {};
    saveSession({ ...current, access_token: accessToken });
    return accessToken;
  },
  ...
  redirectToLogin: (returnTo) => {
    ...
    if (returnTo) url.searchParams.set("from_url", returnTo);
    window.location.href = url.toString();
  },

  // Used by the OAuth callback page and internally after login/refresh.
...
const integrations = {
  Core: {
    UploadFile: async ({ file }) => {
      ...
      return apiFetch("/api/integrations/upload-file", { method: "POST", body: form, isForm: true });
    },
    InvokeLLM: async ({ prompt, system, response_json_schema }) => {
      const data = await apiFetch("/api/integrations/invoke-llm", {
        method: "POST",
        body: { prompt, system, response_json_schema },
      });
      return data.response;
    },
    SendEmail: async ({ to, subject, body }) =>
      apiFetch("/api/integrations/send-email", { method: "POST", body: { to, subject, body } }),
    TranscribeAudio: async ({ audio_url }) => {
      const data = await apiFetch("/api/integrations/transcribe-audio", { method: "POST", body: { audio_url } });
      return data.text;
    },
  },
};
